...
export async function addTool(tool: DBTool): Promise<{ success: boolean; error?: string }> {
...
export async function updateToolBySlug(
  ...
  if (error) {
    console.error(`Error updating tool with slug ${slug}:`, error)
    return { success: false, error: error.message }
  }
...
export async function deleteToolBySlug(slug: string): Promise<{ success: boolean; error?: string }> {
  const { error } = await supabase
    .from("tools")
    .delete()
    .eq("slug", slug)
  ...
  if (error) {
    console.error(`Error deleting tool with slug ${slug}:`, error)
    return { success: false, error: error.message }
  }
...
export async function updateToolStatusBySlug(
  slug: string,
  status: MCPToolStatus
): Promise<{ success: boolean; error?: string }> {
  return updateToolBySlug(slug, { status })
}

export async function updateGitHubStatusBySlug(
  slug: string,
  github_status: GitHubStatus,
  last_github_check_at: string | null = new Date().toISOString().slice(0, 10)
): Promise<{ success: boolean; error?: string }> {
  return updateToolBySlug(slug, { github_status, last_github_check_at })
}

export async function markToolArchived(slug: string): Promise<{ success: boolean; error?: string }> {
  return updateToolBySlug(slug, {
    status: "archived",
    last_github_check_at: new Date().toISOString().slice(0, 10),
  })
}

export async function addFilesystemTool() {
  return addTool({
    slug: "filesystem-mcp",
    name: "Filesystem MCP Server",
    short_description: "Secure file system access for AI agents with configurable permissions",
    answer_first_summary: "Filesystem MCP Server by Anthropic lets AI models read, write, and search files on your local machine through a controlled permission system. It supports directory listing, file creation, and glob-based search patterns, making it essential for coding assistants that need to explore and modify project files safely.",
    developer: "Anthropic",
    github_url: "https://github.com/modelcontextprotocol/servers/tree/main/src/filesystem",
    github_status: "ok",
    last_github_check_at: "2026-06-22",
...
export async function addGithubTool() {
  return addTool({
    slug: "github-mcp",
    name: "GitHub MCP Server",
    short_description: "Full GitHub API integration for repositories, issues, and pull requests",
    ...
    developer: "Anthropic",
    github_url: "https://github.com/modelcontextprotocol/servers-archived/tree/main/src/github",
    github_status: "ok",
    last_github_check_at: "2026-06-22",
...
export async function addPuppeteerTool() {
  return addTool({
    slug: "puppeteer-mcp",
    name: "Puppeteer MCP Server",
    short_description: "Browser automation for web scraping, screenshots, and PDF generation",
    answer_first_summary: "Puppeteer MCP Server gives AI agents the ability to control a headless Chrome browser for web scraping, taking screenshots, generating PDFs, clicking elements, filling forms, and navigating pages. It bridges the gap between AI reasoning and real-time web interaction, enabling tasks like automated testing and data extraction from dynamic websites.",
    developer: "Anthropic",
...
export async function addBraveSearchTool() {
  return addTool({
    slug: "brave-search-mcp",
    name: "Brave Search MCP Server",
    short_description: "Web search powered by Brave Search API for real-time information retrieval",
